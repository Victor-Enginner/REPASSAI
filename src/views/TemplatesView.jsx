...
function BlocoPrompt({ titulo, conteudo, monospace = false }) {
  ...
  return (
    <div style={{ border: '0.5px solid rgba(255,255,255,0.12)', borderRadius: '4px', background: '#0a0e1a' }}>
      <div style={{
        display: 'flex', justifyContent: 'space-between', alignItems: 'center',
        padding: '10px 14px', borderBottom: '0.5px solid rgba(255,255,255,0.1)', gap: '10px',
      }}>
        <span className="mono-label" style={{ fontSize: '10px', color: '#64748b' }}>{titulo}</span>
  ...
  return (
    <button
      ...
      style={{
        ...
        background: '#0a0e1a', border: '0.5px solid rgba(255,255,255,0.14)',
    ...
    >
      ...
      <div style={{ position: 'relative', height: '190px', overflow: 'hidden', background: '#05070f' }}>
        <iframe
        ...
        <span style={{
          ...
          background: 'rgba(5,7,15,0.86)', color: '#a5b4fc',
      ...
      <div style={{ padding: '14px 16px', flex: 1, display: 'flex', flexDirection: 'column', gap: '8px' }}>
        <h3 className="font-headline" style={{ fontSize: '14.5px', color: '#fff', lineHeight: 1.25, margin: 0 }}>
          {template.titulo}
        </h3>
        ...
        <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap', marginTop: 'auto', paddingTop: '8px' }}>
          {(template.tecnologias || []).slice(0, 4).map((t) => (
            <span key={t} style={{
              fontSize: '9.5px', color: '#64748b', border: '0.5px solid rgba(255,255,255,0.14)',
          ...
          <>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '20px', flexWrap: 'wrap', marginBottom: '22px' }}>
              <div style={{ flex: '1 1 420px' }}>
                <h1 className="font-headline" style={{ fontSize: 'clamp(22px, 4vw, 30px)', color: '#fff', letterSpacing: '-0.03em', margin: 0, lineHeight: 1.15 }}>
                  {detalhe.titulo}
                </h1>
                <p style={{ fontSize: '13px', color: '#94a3b8', marginTop: '10px', lineHeight: 1.6, maxWidth: '68ch' }}>
                  {detalhe.descricao}
                </p>
              </div>
            ...
            {/* Preview / Código */}
            <div style={{ border: '0.5px solid rgba(255,255,255,0.14)', borderRadius: '6px', overflow: 'hidden', marginBottom: '22px', background: '#0a0e1a' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', borderBottom: '0.5px solid rgba(255,255,255,0.1)', gap: '10px', flexWrap: 'wrap' }}>
                ...
                <div style={{ display: 'flex', gap: '4px', background: '#05070f', padding: '3px', borderRadius: '4px' }}>
              ...
              {abaPreview === 'preview' ? (
                <iframe
                  ...
                  style={{ width: '100%', height: '540px', border: 0, display: 'block', background: '#fff' }}
                />
              ) : (
                <div style={{ padding: '14px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px', gap: '10px', flexWrap: 'wrap' }}>
                    <span className="mono-label" style={{ fontSize: '10px', color: '#64748b' }}>
                  ...
                  <pre style={{
                    background: '#05070f', border: '0.5px solid rgba(255,255,255,0.12)',
                    padding: '14px', fontSize: '11.5px', color: '#a5b4fc',
                    fontFamily: 'var(--font-mono, monospace)', whiteSpace: 'pre-wrap',
                    overflowX: 'auto', margin: 0, borderRadius: '4px',
                  }}>
                    {detalhe.comando_instalacao}
                  </pre>
                  <p style={{ fontSize: '11px', color: '#64748b', marginTop: '10px', lineHeight: 1.6 }}>
                    Substitua <code style={{ color: '#cbd5e1' }}>SEU_TOKEN</code> pelo token do registry.
            ...
            <div style={{ border: '0.5px solid rgba(255,255,255,0.14)', borderRadius: '6px', background: '#0a0e1a', overflow: 'hidden' }}>
              ...
              <div style={{ padding: '16px' }}>
                {abaInfo === 'instrucoes' ? (
                  ...
                  <div style={{ display: 'grid', gap: '20px', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))' }}>
                    <div>
                      <div className="mono-label" style={{ fontSize: '10px', color: '#64748b', marginBottom: '10px' }}>TECNOLOGIAS</div>
                      <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                        {(detalhe.tecnologias || []).map((t) => (
                          <span key={t} style={{ fontSize: '10.5px', color: '#a5b4fc', border: '0.5px solid rgba(99,102,241,0.4)', padding: '4px 9px', borderRadius: '3px', fontFamily: 'var(--font-mono, monospace)' }}>
                            {t}
                          </span>
                        ))}
                      </div>
                    ...
                    <div>
                      <div className="mono-label" style={{ fontSize: '10px', color: '#64748b', marginBottom: '10px' }}>TIPOGRAFIA</div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                        {(detalhe.fontes || []).length
                          ? detalhe.fontes.map((f) => <span key={f} style={{ fontSize: '11.5px', color: '#cbd5e1' }}>{f}</span>)
                          : <span style={{ fontSize: '11.5px', color: '#64748b' }}>Nenhuma fonte externa</span>}
                      </div>
                    </div>

                    <div>
                      <div className="mono-label" style={{ fontSize: '10px', color: '#64748b', marginBottom: '10px' }}>PALETA DETECTADA</div>
                    ...
                    <div>
                      <div className="mono-label" style={{ fontSize: '10px', color: '#64748b', marginBottom: '10px' }}>REQUISITOS</div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                        {(detalhe.requisitos || []).map((r) => (
                          <span key={r} style={{ fontSize: '11.5px', color: '#cbd5e1' }}>→ {r}</span>
                        ))}
                      </div>
                    </div>
                  </div>
  ...
  return (
    <div style={{ padding: 'clamp(20px, 4vw, 32px) clamp(16px, 4vw, 40px)', maxWidth: '1400px', margin: '0 auto', animation: 'fadeIn 0.25s ease' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '20px', flexWrap: 'wrap', marginBottom: '24px' }}>
        <div>
          <span className="mono-label" style={{ color: '#6366f1' }}>MODULE // TEMPLATES_10</span>
          <h1 className="font-headline" style={{ fontSize: 'clamp(23px, 4.5vw, 32px)', color: '#fff', marginTop: '8px', letterSpacing: '-0.03em' }}>
            LOJA DE TEMPLATES
          </h1>
          <p style={{ fontSize: '13px', color: '#94a3b8', marginTop: '6px', maxWidth: '62ch', lineHeight: 1.6 }}>
            Landing pages prontas com preview ao vivo, prompts de integração e download do código.
          </p>
        </div>

        <div style={{ background: '#0a0e1a', border: '0.5px solid rgba(255,255,255,0.14)', padding: '10px 18px', borderRadius: '4px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '12px', marginBottom: '22px' }}>
        <div style={{ position: 'relative' }}>
          ...
          <input
            ...
            style={{
              width: '100%', padding: '11px 12px 11px 34px', background: '#0a0e1a',
              border: '0.5px solid rgba(255,255,255,0.14)', borderRadius: '4px',
              color: '#fff', fontSize: '12.5px', fontFamily: 'inherit',
            }}
          />
        </div>
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
          ...
          */}
          <textarea
            ...
            style={{
              flex: 1, minWidth: 0, padding: '11px 12px', background: '#0a0e1a',
              border: '0.5px solid rgba(255,255,255,0.14)', borderRadius: '4px',
              color: '#fff', fontSize: '12.5px', fontFamily: 'var(--font-mono, monospace)',
      ...
      {resumoImport && (
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center', background: 'rgba(34,197,94,0.07)', border: '0.5px solid rgba(34,197,94,0.35)', borderRadius: '4px', padding: '12px 14px', marginBottom: '16px' }}>
          ...
          <span style={{ fontSize: '12.5px', color: '#86efac' }}>{resumoImport}</span>
        </div>
      )}
      ...
      {erro && (
        <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-start', background: 'rgba(239,68,68,0.07)', border: '0.5px solid rgba(239,68,68,0.35)', borderRadius: '4px', padding: '14px', marginBottom: '20px' }}>
          ...
          <span style={{ fontSize: '12.5px', color: '#fca5a5', lineHeight: 1.6 }}>{erro}</span>
        </div>
      )}
      ...
      {carregando ? (
        ...
        <div style={{ padding: '56px 24px', textAlign: 'center', border: '0.5px dashed rgba(255,255,255,0.2)', borderRadius: '6px', background: '#0a0e1a' }}>
          <LayoutTemplate size={26} color="#475569" />
          <p style={{ fontSize: '13.5px', color: '#94a3b8', marginTop: '14px' }}>
          ...
          {!busca && (
            <p style={{ fontSize: '12px', color: '#64748b', marginTop: '8px', lineHeight: 1.6 }}>
              Cole o comando <code style={{ color: '#a5b4fc' }}>npx shadcn@latest add …</code> no campo acima
      ...
      <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-start', marginTop: '28px', padding: '14px', background: '#0a0e1a', border: '0.5px solid rgba(255,255,255,0.12)', borderRadius: '4px' }}>
        <Cpu size={15} color="#6366f1" style={{ flexShrink: 0, marginTop: '2px' }} />
        <span style={{ fontSize: '11.5px', color: '#64748b', lineHeight: 1.65 }}>
